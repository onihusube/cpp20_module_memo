...
- モジュールインターフェース単位の本文内（かつプライベートモジュールフラグメントの外側）、もしくはモジュールパーティションで宣言されている
...
`export`宣言内で宣言されていて名前空間スコープに存在するか、次のいずれかに該当する場合に、その宣言は __エクスポート__ される（*exported*）
